$(function() {
	$.fn.smohanfacebox = function(options) {
	    ...
	    for(i=0;i<60;i++){  //通过循环创建60个表情，可扩展
		 faceimg+='<li><a href="javascript:void(0)"><img src="' + WEB_URL +'/static/plugins/face/images/face/'+(i+1)+'.gif" face="<emt>'+(i+1)+'</emt>"/></a></li>';
		};
 ...
 $.fn.extend({
	  replaceface : function(faces){
		  for(i=0;i<60;i++){
			  faces=faces.replace('<emt>'+ (i+1) +'</emt>','<img src="' + WEB_URL + '/static/plugins/face/images/face/'+(i+1)+'.gif">');
			  }
		   $(this).html(faces);
		   }
	  });
